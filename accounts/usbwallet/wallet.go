...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the IrChain
...
type wallet struct {
	...
	deriveChain    irchain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived IrChain account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain irchain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
