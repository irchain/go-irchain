...
func (irc *IrChain) startBloomHandlers() {
			...
			for {
				...
				case request := <-irc.bloomRequests:
					...
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(irc.chainDb, (section+1)*params.BloomBitsBlocks-1)
						if compVector, err := rawdb.ReadBloomBits(irc.chainDb, task.Bit, section, head); err == nil {
...
// for the IrChain header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  ircdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db ircdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := ircdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
