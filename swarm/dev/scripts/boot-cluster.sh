...
#
# The cluster contains a bootnode, a ghuc node and multiple swarm nodes, with
...
BOOTNODE_URL="hnode://${BOOTNODE_PUBKEY}@${BOOTNODE_IP}:${BOOTNODE_PORT}"

# static ghuc configuration
GHUC_IP="192.168.33.3"
GHUC_RPC_PORT="8545"
GHUC_RPC_URL="http://${GHUC_IP}:${GHUC_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the ghuc and
# swarm nodes
start_bootnode() {
...
# start_ghuc_node starts a ghuc node with --datadir pointing at <base-dir>/ghuc
# and a single, unlocked account with password "ghuc"
start_ghuc_node() {
  local dir="${base_dir}/ghuc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get ghuc account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GHUC_IP}"
    --rpcport   "${GHUC_RPC_PORT}"
    --verbosity "6"
  )

  start_node "ghuc" "${GHUC_IP}" "$(which ghuc)" ${args[@]}
}
...
create_account() {
  ...
  ghuc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
