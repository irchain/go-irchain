...
// Reference: https://github.com/ethereum/wiki/wiki/Huchash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see huchash_io_prepare
enum huchash_io_rc {
	HUCHASH_IO_FAIL = 0,           ///< There has been an IO failure
	HUCHASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	HUCHASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	HUCHASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of huchash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * huchash_full_t
 */
#ifdef HUCHASH_PRINT_CRITICAL_OUTPUT
#define HUCHASH_CRITICAL(...)							\
	do													\
	{													\
		printf("HUCHASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for huchash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the huchash
 ...
 *                           https://github.com/ethereum/wiki/wiki/Huchash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum huchash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/ethereum/wiki/wiki/Huchash-DAG
 *
...
static inline bool huchash_io_mutable_name(
	uint32_t revision,
