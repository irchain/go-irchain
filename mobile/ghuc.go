...
type NodeConfig struct {
	...
	// HappyUCEnabled specifies whether the node should run the HappyUC protocol.
	HappyUCEnabled bool

	// HappyUCNetworkID is the network identifier used by the HappyUC protocol to
	// decide if remote peers should be accepted or not.
	HappyUCNetworkID int64 // uint64 in truth, but Java can't handle that...

	// HappyUCGenesis is the genesis JSON to use to seed the blockchain with. An
	// empty genesis state is equivalent to using the mainnet's state.
	HappyUCGenesis string

	// HappyUCDatabaseCache is the system memory in MB to allocate for database caching.
	// A minimum of 16MB is always reserved.
	HappyUCDatabaseCache int

	// HappyUCNetStats is a netstats connection string to use to report various
	// chain, transaction and node stats to a monitoring server.
	//
	// It has the form "nodename:secret@host:port"
	HappyUCNetStats string
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.HappyUCGenesis != "" {
		...
		if err := json.Unmarshal([]byte(config.HappyUCGenesis), genesis); err != nil {
			return nil, fmt.Errorf("invalid genesis spec: %v", err)
		}
		// If we have the testnet, hard code the chain configs too
		if config.HappyUCGenesis == TestnetGenesis() {
			genesis.Config = params.TestnetChainConfig
			if config.HappyUCNetworkID == 1 {
				config.HappyUCNetworkID = 3
			}
		}
	}
...
// GetHappyUCClient retrieves a client to access the HappyUC subsystem.
func (n *Node) GetHappyUCClient() (client *HappyUCClient, _ error) {
