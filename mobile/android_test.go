...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.happyuc", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for happyuc-go issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/happyuc/gethtest", "libs"} {
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "happyuc", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
