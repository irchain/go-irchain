...
// Package girc contains the simplified mobile APIs to go-irchain.
//
// The scope of this package is *not* to allow writing a custom IrChain client
// with pieces plucked from go-irchain, rather to allow writing native dapps on
...
// from the go-irchain repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-irchain version.
