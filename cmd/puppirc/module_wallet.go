...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'girc --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'girc --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ircstats \'{{.Ircstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh

RUN \
	sed -i 's/PuppethNetworkID/{{.NetworkID}}/g' dist/js/irchainwallet-master.js && \
	sed -i 's/PuppethNetwork/{{.Network}}/g'     dist/js/irchainwallet-master.js && \
	sed -i 's/PuppethDenom/{{.Denom}}/g'         dist/js/irchainwallet-master.js && \
	sed -i 's/PuppethHost/{{.Host}}/g'           dist/js/irchainwallet-master.js && \
	sed -i 's/PuppethRPCPort/{{.RPCPort}}/g'     dist/js/irchainwallet-master.js
