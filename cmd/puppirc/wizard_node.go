...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.ircstats == "" {
		log.Error("No ircstats server configured")
		return
	}
	...
	if w.conf.Genesis.Config.Irchash != nil && !boot {
		fmt.Println()
		if infos.irchashdir == "" {
			fmt.Printf("Where should the irchash mining DAGs be stored on the remote machine?\n")
			infos.irchashdir = w.readString()
		} else {
			fmt.Printf("Where should the irchash mining DAGs be stored on the remote machine? (default = %s)\n", infos.irchashdir)
			infos.irchashdir = w.readDefaultString(infos.irchashdir)
		}
	}
	...
	if infos.ircstats == "" {
		...
		infos.ircstats = w.readString() + ":" + w.conf.ircstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.ircstats)
		infos.ircstats = w.readDefaultString(infos.ircstats) + ":" + w.conf.ircstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.Irchash != nil {
			// Irchash based miners only need an coinbase to mine against
			fmt.Println()
			if infos.coinbase == "" {
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy IrChain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
