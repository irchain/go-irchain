...
// returned from docker inspect, parsed into a form easily usable by puppirc.
type containerInfos struct {
	running bool              // Flag whircer the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whircer there's a container running for the service
