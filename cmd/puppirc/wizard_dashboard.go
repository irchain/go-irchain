...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ircstats", "explorer", "wallet", "faucet"} {
	...
	// If we have ircstats running, ask whether to make the secret public or not
	if w.conf.ircstats != "" {
		fmt.Println()
		fmt.Println("Include ircstats secret on dashboard (y/n)? (default = yes)")
