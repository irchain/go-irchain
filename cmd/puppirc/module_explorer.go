...
FROM puppirc/explorer:latest

ADD ircstats.json /ircstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../irc-net-intelligence-api && pm2 start /ircstats.json)' >  explorer.sh && \
	echo '(cd ../irchainchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerIRCstats is the configuration file for the ircstats javascript client.
var explorerIrcstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.irchain
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Ircstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	ircstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerIrcstats)).Execute(ircstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.ircstats[:strings.Index(config.ircstats, ":")],
		"Secret": config.ircstats[strings.Index(config.ircstats, ":")+1 : strings.Index(config.ircstats, "@")],
		"Host":   config.ircstats[strings.Index(config.ircstats, "@")+1:],
	})
	files[filepath.Join(workdir, "ircstats.json")] = ircstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.irchain"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		ircstats: infos.envvars["STATS"],
	}
	return stats, nil
}
