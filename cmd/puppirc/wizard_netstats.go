...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ircstats availability")
	if infos, err := checkIrcstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ircstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ircstats"] = infos.Report()
		ircstats = infos.config
	}
