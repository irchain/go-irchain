...
type config struct {
	...
	ircstats  string   // Ircstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // IrChain services known to be running on servers
...
// converts it to an IrChain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by ircstats, which compares IPs textually, not by value.
