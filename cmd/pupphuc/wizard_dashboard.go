...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"hucstats", "explorer", "wallet", "faucet"} {
	...
	// If we have hucstats running, ask whether to make the secret public or not
	if w.conf.hucstats != "" {
		fmt.Println()
		fmt.Println("Include hucstats secret on dashboard (y/n)? (default = yes)")
