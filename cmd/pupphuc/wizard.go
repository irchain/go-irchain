...
type config struct {
	...
	hucstats  string   // Hucstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // HappyUC services known to be running on servers
...
// converts it to an HappyUC address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by hucstats, which compares IPs textually, not by value.
