...
// nodeDockerfile is the Dockerfile required to run an HappyUC node.
var nodeDockerfile = `
FROM happyuc/client-go:latest
...
{{end}}
RUN \
  echo 'ghuc --cache 512 init /genesis.json' > ghuc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.happyuc/keystore/ && cp /signer.json /root/.happyuc/keystore/' >> ghuc.sh && \{{end}}
	echo $'ghuc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --hucstats \'{{.Hucstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Coinbase}}--coinbase {{.Coinbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> ghuc.sh

ENTRYPOINT ["/bin/sh", "ghuc.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an HappyUC node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new HappyUC node container to a remote machine via SSH,
...
func (info *nodeInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Hucstats username":        info.hucstats,
	}
	if info.gasTarget > 0 {
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 ghuc --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		...
		hucstats:   infos.envvars["STATS_NAME"],
