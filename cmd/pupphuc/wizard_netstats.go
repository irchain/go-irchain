...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for hucstats availability")
	if infos, err := checkHucstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["hucstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["hucstats"] = infos.Report()
		hucstats = infos.config
	}
	...
	if infos, err := checkNode(client, w.network, true); err != nil {
		...
		bootnodes = append(bootnodes, infos.hnode)
	}
