...
func (w *wizard) deployNode(boot bool) {
	...
	if infos.hucstats == "" {
		...
		infos.hucstats = w.readString() + ":" + w.conf.hucstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.hucstats)
		infos.hucstats = w.readDefaultString(infos.hucstats) + ":" + w.conf.hucstats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy HappyUC node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
