...
FROM pupphuc/explorer:latest

ADD hucstats.json /hucstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../huc-net-intelligence-api && pm2 start /hucstats.json)' >  explorer.sh && \
	echo '(cd ../happyucchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerHucstats is the configuration file for the hucstats javascript client.
var explorerHucstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.happyuc
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.Hucstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	hucstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerHucstats)).Execute(hucstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.hucstats[:strings.Index(config.hucstats, ":")],
		"Secret": config.hucstats[strings.Index(config.hucstats, ":")+1 : strings.Index(config.hucstats, "@")],
		"Host":   config.hucstats[strings.Index(config.hucstats, "@")+1:],
	})
	files[filepath.Join(workdir, "hucstats.json")] = hucstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.happyuc"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		hucstats: infos.envvars["STATS"],
	}
	return stats, nil
}
