...
func loadConfig(file string, cfg *ghucConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "huc", "shh")
	cfg.WSModules = append(cfg.WSModules, "huc", "shh")
	cfg.IPCPath = "ghuc.ipc"
	return cfg
}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the HappyUC Stats daemon if requested.
	if cfg.Hucstats.URL != "" {
		utils.RegisterHucStatsService(stack, cfg.Hucstats.URL)
	}
	return stack
}
