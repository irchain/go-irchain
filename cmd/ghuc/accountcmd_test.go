...
func TestAccountListEmpty(t *testing.T) {
	ghuc := runGhuc(t, "account", "list")
	ghuc.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	ghuc := runGhuc(t, "account", "list", "--datadir", datadir)
	defer ghuc.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	ghuc := runGhuc(t, "account", "new", "--lightkdf")
	defer ghuc.ExpectExit()
	ghuc.Expect(`
...
`)
	ghuc.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	ghuc := runGhuc(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer ghuc.ExpectExit()
	ghuc.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(ghuc.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(ghuc.StderrText(), m) {
...
// https://github.com/happyuc-project/happyuc-go/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	ghuc.SetTemplateFunc("keypath", func(file string) string {
