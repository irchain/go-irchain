...
// ghuc is the official command-line client for HappyUC.
package main
...
const (
	clientIdentifier = "ghuc" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the happyuc-go command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2018 The happyuc-go Authors"
...
// ghuc is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func ghuc(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	go func() {
		...
		stateReader := hucclient.NewClient(rpcClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full HappyUC node is running
