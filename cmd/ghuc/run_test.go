...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "ghuc-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "ghuc-test" in runGhuc.
	reexec.Register("ghuc-test", func() {
...
// spawns ghuc with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGhuc(t *testing.T, args ...string) *testghuc {
	tt := &testghuc{}
	...
	// Boot "ghuc". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("ghuc-test", args...)
