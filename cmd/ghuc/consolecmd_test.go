...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a ghuc console, make sure it's cleaned up and terminate the console
	ghuc := runGhuc(t,
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\ghuc` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "ghuc.ipc")
	}
...
func testAttachWelcome(t *testing.T, ghuc *testghuc, endpoint, apis string) {
	// Attach to a running ghuc note and terminate immediately
