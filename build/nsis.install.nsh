Name "ghuc ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install ghuc binary
Section "Ghuc" GHUC_IDX
  SetOutPath $INSTDIR
  file {{.Ghuc}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\ghuc.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\ghuc.exe" "attach" "" ""
  ...
  ${EnvVarUpdate} $0 "HAPPYUC_SOCKET" "A" "HKLM" "\\.\pipe\ghuc.ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GHUC_IDX}
    SectionGetSize ${GHUC_IDX} $0
