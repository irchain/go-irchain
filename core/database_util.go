...
func WriteCanonicalHash(db hucdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db hucdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db hucdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db hucdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db hucdb.Putter, count uint64) error {
...
func WriteHeader(db hucdb.Putter, header *types.Header) error {
...
func WriteBody(db hucdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db hucdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db hucdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db hucdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db hucdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db hucdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db hucdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db hucdb.Database) hucdb.Database {
	return hucdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db hucdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db hucdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db hucdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
