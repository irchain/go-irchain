...
var (
	...
	// err = core.ErrInsufficientFees is returned if the total cost of executing a transaction
	// is higher than the balance of the user's account.
	ErrInsufficientFeesByBalance = errors.New("insufficient balance for gas * price")

	// ErrInsufficientFeesByContract is returned if the total cost of executing a transaction
	// is higher than the balance of the user's account.
	ErrInsufficientFeesByContract = errors.New("insufficient contract balance for gas * price")
...
func (pool *TxPool) validateTx(tx *types.Transaction, local bool) error {
	...
	// // Creating contract need to store a certain amount of ircer
	...
	if len(tx.Data()) == 0 {
		assert = tx.Value()
		err = ErrInsufficientFees
	} else if contractCreation {
		assert = pool.currentState.GetBalance(from)
		err = ErrInsufficientFeesByBalance
	} else {
		assert = pool.currentState.GetBalance(*tx.To())
		err = ErrInsufficientFeesByContract
	}
