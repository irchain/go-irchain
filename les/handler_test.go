...
func TestTransactionStatusLes2(t *testing.T) {
	...
	gchain, _ := core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), hucash.NewFaker(), db, 1, func(i int, block *core.BlockGen) {
		block.AddTx(tx1)
		block.AddTx(tx2)
	})
	...
	gchain, _ = core.GenerateChain(params.TestChainConfig, chain.GetBlockByNumber(0), hucash.NewFaker(), db, 2, func(i int, block *core.BlockGen) {})
	if _, err := chain.InsertChain(gchain); err != nil {
		panic(err)
	}
