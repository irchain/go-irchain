	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = huchash.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (huchash *Huchash) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
	...
	return huchash.verifyHeader(chain, headers[index], parent, false, seals[index])
}
...
func (huchash *Huchash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := huchash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func (huchash *Huchash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	expected := huchash.CalcDifficulty(chain, header.Time.Uint64(), parent)
	...
	if seal {
		if err := huchash.VerifySeal(chain, header); err != nil {
			return err
		}
	}
...
func (huchash *Huchash) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	if huchash.config.PowMode == ModeFake || huchash.config.PowMode == ModeFullFake {
		...
		if huchash.fakeFail == header.Number.Uint64() {
	...
	if huchash.shared != nil {
		return huchash.shared.VerifySeal(chain, header)
	}
	...
	cache := huchash.cache(number)
	size := datasetSize(number)
	if huchash.config.PowMode == ModeTest {
		size = 32 * 1024
	}
...
func (huchash *Huchash) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	header.Difficulty = huchash.CalcDifficulty(chain, header.Time.Uint64(), parent)
	return nil
}
