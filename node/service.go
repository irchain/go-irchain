...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (ircdb.Database, error) {
	if ctx.config.DataDir == "" {
		return ircdb.NewMemDatabase(), nil
	}
	db, err := ircdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
